...
export default function AdminPanel({ adminData, setAdminData }) {
  ...
  function changeImageCategory(id, cat) {
    update(
      "images",
      adminData.images.map((i) => (i.id === id ? { ...i, category: cat } : i))
    );
  }

  function addVideo(url, place) {
    update("videos", [...adminData.videos, { id: Date.now(), url, place }]);
  }
            ...
            {tab === "about" && (
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => updateNested("about.text", e.target.value)}
                />
              </div>
            )}
...
function VideoManager({ addVideo, videos, removeVideo }) {
  ...
  const [place, setPlace] = useState("Portfolio");
