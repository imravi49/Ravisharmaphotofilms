...
export default function AdminPanel({ adminData, setAdminData }) {
  ...
  function changeImageCategory(id, cat) {
    update(
      'images',
      adminData.images.map((i) =>
        i.id === id ? { ...i, category: cat } : i
      )
    );
  }
  ...
  function addVideo(url, place) {
    update('videos', [
      ...adminData.videos,
      { id: Date.now() + Math.random(), url, place },
    ]);
  }
          ...
          <div style={{ marginTop: 12 }}>
            {tab === 'general' && (
              <div>
                ...
                <input
                  value={adminData.heroTag}
                  onChange={(e) =>
                    update('heroTag', e.target.value)
                  }
                />
              </div>
            )}
            ...
            {tab === 'about' && (
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateNested('about.title', e.target.value)
                  }
                />
                <label>About Text</label>
                <textarea
                  value={adminData.about.text}
                  onChange={(e) =>
                    updateNested('about.text', e.target.value)
                  }
                />
              </div>
            )}
            ...
            {tab === 'images' && (
              <div>
                ...
                <div
                  {...getRootProps()}
                  style={{
                    border: '2px dashed rgba(197,166,84,0.3)',
                    padding: 12,
                    borderRadius: 8,
                    cursor: 'pointer',
                  }}
                    ...
                    >
                      ...
                      >
                        <select
                          value={img.category}
                          onChange={(e) =>
                            changeImageCategory(img.id, e.target.value)
                          }
                        >
            ...
            {tab === 'seo' && (
              <div>
                ...
                <input
                  value={adminData.seo.title}
                  onChange={(e) =>
                    updateNested('seo.title', e.target.value)
                  }
                />
                <label>Meta Description</label>
                <textarea
                  value={adminData.seo.description}
                  onChange={(e) =>
